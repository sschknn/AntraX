...
export const ProductList: React.FC<ProductListProps> = ({ products, lang, onQuickView, onAddToCart }) => {
  ...
  const lookTotal = products.reduce((sum, p) => sum + p.priceValue, 0);
    ...
    <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex items-center justify-between px-2">
        <div className="flex flex-col">
          ...
          <span className="text-[12px] font-black text-white mt-1">€{lookTotal.toFixed(2)} <span className="text-[8px] text-slate-500 uppercase ml-1 font-bold">{t.totalPrice}</span></span>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        {products.map((product) => (
          ...
          >
            <div 
              className="w-16 h-16 bg-slate-800 rounded-2xl flex items-center justify-center text-3xl group-hover:scale-110 transition-transform flex-shrink-0 cursor-pointer"
            ...
            <div className="flex flex-col flex-1 gap-0.5 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-0.5">
                ...
                <span className="text-[7px] bg-amber-500/10 text-amber-500 px-1.5 py-0.5 rounded-full font-black uppercase tracking-widest">Amazon Prime</span>
              </div>
            </div>
